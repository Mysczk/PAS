...
acpass <- table(Cars93$Passengers)
cumpass <- cumsum(acpass)
(rcpass<-round(prop.table(acpass),2))
(krcpass<-cumsum(rcpass))
data.frame(cbind("n(i)"=acpass,"N(i)"=cumpass,"f(i)"=rcpass,"F(i)"=krcpass))

# zmena typu promenne kvuli popiskum na y-ove ose v grafu
x.val<-as.numeric(as.character(names(acpass)))
ac.n<-as.numeric(acpass)
plot(x.val,ac.n,type="h",lwd=3,col="darkgreen",main="Frekvencni polygon",
     xlab="Pocet spolucest",ylab="Absolutni cetnosti")
x.val2<-min(x.val):max(x.val)
ac2<-rep(0,length(x.val2))
for(i in 1:length(x.val2)){
  for(j in 1:length(x.val)){if(x.val2[i]==x.val[j]){
    ac2[i]<-acpass[j]}
    }
}
lines(x.val2,ac2,col="red")
