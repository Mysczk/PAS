...
vaha <- Kojeni$porHmotnost
t.test(vaha, mu=3500)
MeanCI(vaha, conf.level = 0.95) # s vyznamnosti hladinou 5 % muze byt stredni hodnotou 3.5kg

## V jakem rozmezi by se s 90% spolehlivosti mela nachazet stredni hodnota porodni delky deti?
delka <- Kojeni$porDelka
MeanCI(delka, conf.level = 0.9) # s 90% spolehlivosti se nachazi v rozmezi 50.33 cm - 50.85 cm
...
plot(zavisla ~ nezavisla + nezavisla2, pch=19,main="Graf zavislosti dvou ciselnych promennych",
     xlab="Porodni delka",ylab="Porodni hmotnost")
