...
hist(bmi,col="skyblue",border="darkblue",main="Histogram",ylab="Hustota",
     xlab="BMI v kg/m^2",breaks=10,freq=F)
(jadro <- density(bmi))
lines(jadro, col=2)
jadro$x[which.max(jadro$y)]
...
(DM <- mean(vaha) - 3*IQR(vaha))
vaha[vaha<DM]
# dolni adepti na odlehle pozorovani
(HM<-mean(vaha) + 3*IQR(vaha))
vaha[vaha>HM]

(DM <- mean(vaha) - 4*IQR(vaha))
vaha[vaha<DM]
# dolni adepti na odlehle pozorovani
(HM<-mean(vaha) + 4*IQR(vaha))
vaha[vaha>HM]
boxplot(vaha, range = 4)
...
(HM <- quantile(cukr.t,0.25) - 1.5*IQR(cukr.t))
(HM<-quantile(cukr.t,0.75) + 1.5*IQR(cukr.t))
(HM<-mean(cukr.t) + 3*IQR(cukr.t))
(HM<-mean(cukr.t) + 4*IQR(cukr.t))
cukr[cukr>HM]
